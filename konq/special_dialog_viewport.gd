...
func create_entries(tileid, tilepos, map, resources_map):
	clear()
	setup_and_show(
		buildings.TILEID_SCENES[tileid],
		CLASSMAP[buildings.TILEID_CLASSES[tileid]].new(tilepos, map, resources_map).get_args()
	)
